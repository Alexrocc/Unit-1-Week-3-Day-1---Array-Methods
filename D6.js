...
const splicedStrings = (str1, str2) => {
  let slice1 = str1.slice(0, 2)
  let slice2 = str2.slice(str2.length - 3, str2.length)
  let newString = slice1.concat(slice2)
  return newString
}

console.log(splicedStrings("ciao amico", "come stai"))
...
const valueArray = [11, 24, 114, 867, 44, 1, 15, 21, 74]


const evens = valueArray.filter(number => number % 2 === 0)
...
const sumForEach = valueArray.forEach((sum, value => {
  sum += value;
  return sum
} ))
console.log(sum)
...
const sumReduce = valueArray.reduce((total, current) =>{
...
*/
let n = 10
const incrementedValue = valueArray.map(value => value + n)
...
const stringArray = ["Percival", "knight", "sword"]

const chartoNum = stringArray.map(stringLenght => {
...
const oddNumbersto100 = []
  numbersTo100.forEach((num) => {
  if (num % 2 === 1){
    oddNumbersto100.push(num)
  }
  
})
console.log(oddNumbersto100)
...
let oldestMovie = movies.find(item => item.Year === '1963')
console.log(base, oldestMovie)
